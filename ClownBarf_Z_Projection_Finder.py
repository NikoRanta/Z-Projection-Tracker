...
def Z_Projection():
    ...
    if Save_As_Singles_Check.get() == 1:
        ...
        Place_Holder_Zeros = ''
        for x in range(len(str(Reconstruction_Files.shape[1]))-1):
            Place_Holder_Zeros = Place_Holder_Zeros+'0'
        for time_point in range(Reconstruction_Files.shape[1]):
            Reconstruction_Built = np.zeros((Reconstruction_Files.shape[0],Shape_Finding.shape[0],Shape_Finding.shape[1]),'<f4')
            ...
            print(f'Completed Time Point #{time_point+1} ({np.round(timeit.default_timer()-start,3)}) seconds')
            
            print(f'Saving Z Projection tif File time #{time_point+1} ({np.round(timeit.default_timer()-start,3)}) seconds')
            io.imsave(Output_Directory_Text.get()+Z_Project_Value_Name.get()+'_'+Place_Holder_Zeros[:-len(str(time_point+1))]+str(time_point+1)+'.tif',Z_Proj_Value,check_contrast=False)
            print(f'Saving Z Location tif File time #{time_point+1} ({np.round(timeit.default_timer()-start,3)}) seconds')
            io.imsave(Output_Directory_Text.get()+Z_Project_Loc_Name.get()+'_'+Place_Holder_Zeros[:-len(str(time_point+1))]+str(time_point+1)+'.tif',Z_Proj_Loc,check_contrast=False)
            print(f'\nFinished time point #{time_point+1} ({np.round(timeit.default_timer()-start,3)}) seconds\n')
...
if __name__ == '__main__':
    ...
    Spacing_Show = True
    if Spacing_Show:
        Intro_Spacing,Overal_Error_Spacing,Input_Spacing,Output_Spacing,Z_Value_Spacing,Z_Loc_Spacing,Start_Process_Spacing,Label_Button_Horizontal,Entry_Horizontal,Start_Processing_Horizontal,Error_Symbol_Horizontal,gap,Z_Value_Error_Spacing,Z_Loc_Error_Spacing,Input_Output_Error_Horizontal,Output_Filenames_Horizontal = IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar(),IntVar()
